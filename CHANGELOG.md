...
- **The studio as phone apps** (`mobile/`, [docs/MOBILE.md](docs/MOBILE.md)):
  ...
  directory exactly as the desktop shells do. What ships is what the
  website serves, so no feature can exist on one platform and not
  another — there is one implementation of each. A pure native rewrite
  was considered and rejected: the stroke-to-outline geometry already
  has to be kept identical between `web/js/outline.js` and
  `pipeline/json_to_ufo.py`, and making that four or five copies in four
  or five languages would mean any drift silently produces wrong fonts.
  The native projects are generated by `npx cap add`, not committed.
  Built by CI for both platforms; the APK is attached to each release.

### Fixed
- **Export font did nothing at all inside the app**
  (`web/js/savefile.js`): a browser hands over a file with an
  `<a download>` click, and a WebView ignores it — WKWebView drops it
  entirely on a `blob:` URL. So the first iOS build launched, drew
  perfectly, and then silently produced no file when you pressed the one
  button the whole app exists for. No error, no hint. Only running it on
  a simulator could have found this. All three exports — draft TTF,
  single glyph as SVG, project file — now go through one save layer that
  keeps the plain `<a download>` on the web and opens the **native share
  sheet** in the app (Save to Files, AirDrop, Drive). Verified on an
  iPhone 17 simulator: the sheet appears with a 16 KB file iOS
  identifies as a font.
