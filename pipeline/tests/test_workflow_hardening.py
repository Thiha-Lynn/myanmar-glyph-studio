...
@pytest.mark.parametrize("path", WORKFLOWS, ids=lambda p: p.name)
def test_pip_installs_are_hash_verified(path):
    """`pip install fonttools` takes whatever PyPI serves that minute.
    Every install in CI names a compiled requirements file whose pins
    carry hashes, so a tampered or yanked-and-replaced release cannot
    enter a build."""
    for line in path.read_text(encoding="utf-8").splitlines():
        if not re.search(r"\bpip install\b", line):
            continue
        assert "--require-hashes" in line and "-r " in line, (
            "%s: %s — install from a hashed requirements file"
            % (path.name, line.strip()))


def test_every_compiled_requirements_file_has_its_source():
    """The .txt files are generated; editing one by hand is how the pins
    and the hashes drift apart."""
    pipeline = ROOT / "pipeline"
    compiled = sorted(pipeline.glob("requirements*.txt"))
    assert compiled, "no compiled requirements files found"
    for txt in compiled:
        src = txt.with_suffix(".in")
        assert src.exists(), "%s has no .in source" % txt.name
        body = txt.read_text(encoding="utf-8")
        head = body[:400]
        assert "autogenerated by uv" in head, (
            "%s is not a uv output — regenerate it" % txt.name)
        # Resolving on one platform silently drops the dependencies that
        # only another platform needs: keyring pulls SecretStorage on
        # Linux alone, so a lock compiled on macOS installed fine there
        # and failed the Linux runner under --require-hashes.
        assert "--universal" in head, (
            "%s was not resolved universally — a lock compiled for one "
            "platform omits the others' conditional dependencies"
            % txt.name)
        assert "--hash=sha256:" in body, "%s carries no hashes" % txt.name
