...
def test_every_compiled_requirements_file_has_its_source():
    ...
    for txt in compiled:
        ...
        head = txt.read_text(encoding="utf-8")[:400]
        assert "autogenerated by pip-compile" in head, (
            "%s is not a pip-compile output — regenerate it" % txt.name)
        assert "--hash=sha256:" in txt.read_text(encoding="utf-8"), (
            "%s carries no hashes" % txt.name)
