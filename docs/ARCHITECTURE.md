...
| `fetch_tipitaka.py` | `mgs-fetch-tipitaka` | download the 61 canonical books (Burmese script, VRI XML) as validation corpora; `--write-cover` regenerates `pali_corpus.txt` |
| `pali_translit.py` | `mgs-pali` | romanized Pali → Burmese script, verified 62,729/62,729 words against the VRI parallel edition |
| `make_tipitaka.py` | `mgs-tipitaka` | build `web/data/tipitaka.js` for the reader from SuttaCentral's public-domain root text |
